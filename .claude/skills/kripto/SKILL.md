...
6. D2 likidasyon — **SIRA ÖNEMLİ, önce ücretsiz** (2026-09-07):
   **(1) BİRİNCİL — ücretsiz:** `python liqmap_bedava.py --symbol <SYM> --pay 1 [--poz]`
   (Binance aggTrades, **0 USD**, anahtarsız). Yük ölçüldü: ARKM 16 çağrı ~15 sn ·
   ICP 89 · TIA 170 · BTC 698.
   **(2) YEDEK — ücretli, ÖNCE ONAY SOR:** `python apify_liq.py --symbol <SYM>`
   (Apify CoinGlass, **~$0,01/koşu**, FREE plan $5 tavan). **Yalnızca (1) veri
   veremezse** ve **kullanıcı onayladıktan sonra.**
   **(3) SON ÇARE:** web_search.
   ~~[ücretsiz canlı] `python likidasyon.py --dk 10`~~ **(ÖLÜ — aşağıda)**
...
Likidasyon — **önce ücretsiz, sonra ücretli (onayla)**:
**(1)** `liqmap_bedava.py --symbol <SYM> --pay 1` → mıknatıs seviyeleri, **0 USD**, HER analizde kullanılabilir.
**(2)** veri gelmezse → `apify_liq.py --symbol <SYM>` (CoinGlass, ~$0,01) — **ÖNCE KULLANICIYA SOR.**
**(3)** o da olmazsa → swing high/low + web_search.
🔴 **ÇIKTI BAĞLAM KATMANIDIR, SİNYAL DEĞİL.** Harita = büyük işlemlerin fiyat histogramının kaldıraç sabitleriyle (×0,99/0,98/0,96/0,90) kaydırılmış hâli; girdisi yalnız fiyat+hacim. Ölçülmüş bir kenar YOK → **D1/D2/D4 sinyal sayımına GİRMEZ**, yalnız stop yerleşimi/stop-hunt bölgesi yorumunda kullanılır.
...
> **İzleme:** `olcucu.py --symbol <SYM> --mtf` → çoklu-TF + erken belirti. Likidasyon = **önce `liqmap_bedava.py` (ücretsiz)**, veri gelmezse **onay alarak** `apify_liq.py` (~$0,01), sonra web_search.
...
- **LİQ-MAP (D2) — ÜÇ KADEMELİ, 2026-09-07 kullanıcı kararı:**
  **(1) BİRİNCİL, ÜCRETSİZ:** `python "c:/Users/alper/Desktop/kripto trade/liqmap_bedava.py" --symbol <SYM> --pay 1 [--saat 24] [--poz]` → Binance aggTrades'ten mıknatıs seviyeleri. **0 USD, anahtarsız.** `--pay` eşiği veriden alır (sembolden sembole ölçeklenir); sabit `--esik` BTC'ye göredir, altlarda hiçbir işlem geçemeyebilir. `--poz` açık pozisyonun stop/tetik/hedefiyle hizalar.
  **(2) YEDEK, ÜCRETLİ:** `python ".../apify_liq.py" --symbol <SYM> [--interval 24h]` → CoinGlass. **~$0,01/koşu, FREE plan $5 tavan.** 🔴 **YALNIZCA (1) veri veremezse VE kullanıcı ONAY verirse.** Token: `CONFIG.apify_token`.
  ⚠️ `apify_liq.py`'de bilinen kusur: küresel en büyük 12 kümeyi alıp fiyata göre bölüyor → bir taraf BOŞ görünebilir ("orada mıknatıs yok" sanılır). `liqmap_bedava.py` üstten ve alttan AYRI AYRI alır.
  ⚠️ İkisi AYNI SINIF ama AYNI SAYI DEĞİL (ICP kıyası: CoinGlass −12..−13% 18-28M · bedava −0,5..−9,5% 0,9-1,3M) — pencere/kademe/ağırlık farklı.
...
- **LİKİDASYON AKIŞI (D2) — 🔴 ÖLÜ/RAFTA (2026-07-23):** `likidasyon.py` (Binance forceOrder stdlib WS) bu bölgede **engelli**: 0 olay, `likidasyon_log.jsonl` hep boştu → `arsiv/`'e taşındı. Alternatif `coinalyze.py` (REST liq geçmişi) denendi ama forward-return **N=605'te edge YOK** → `arsiv/`'e taşındı (rafta). **Bot likidasyon verisi kullanmıyor.** Harita/mıknatıs gerekirse **önce `liqmap_bedava.py` (ücretsiz)**, veri gelmezse **onayla** `apify_liq.py`; sonra web_search. Gerekçe detayı: `fikir-defteri.md`.
  🔴 **YENİDEN ÖLÇÜLDÜ 2026-09-07** (kayıt tekrarlanmadı, ölçüldü): forceOrder akışı TCP/TLS **bağlanıyor**, handshake **HTTP/1.1 101** dönüyor, Binance **ping** gönderip bağlantıyı canlı tutuyor — ama **423 saniyede 0 olay**. Yani "bölge engeli" değil, **sessiz akış**: kabul ediyor, veri vermiyor. Aynı saatlerde Coinalyze BTC'de likidasyon görüyor. → `binance-liquidation-tracker` türü repolar burada **çalışmaz**.
  🔴 **Coinalyze yeniden ölçüldü 2026-09-07:** likidasyon DENGESİZLİĞİ (short−long)/(short+long), N=1.614 · 71 gün → **işaret keşif/holdout arasında DÖNDÜ** (+4,65% → −3,19%). Karıştırıcı kontrolü GEÇTİ (fiyatın kılığı değil) ama **kararlı yön YOK**. Büyüklük (toplam USD) hiçbir şey taşımıyor — emir defteri dersinin aynısı.
