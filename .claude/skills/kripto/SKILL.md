...
**Gemma/Ollama (D4 haber özeti — aktif: `gemma4:12b`):**
- `POST {ollama_url}/api/generate` body: `{model, "think":false, "stream":false, "options":{"num_predict":220}, prompt:"<haber metni> → 2-3 madde Türkçe özet + sonuna BOĞA/AYI/NÖTR"}`
- ⚠️ gemma4:12b **düşünen** modeldir; `"think":false` verilmezse `response` boş döner. Apostrof/özel karakter içeren prompt'u shell'e gömme — payload'ı dosyaya yazıp `--data-binary @dosya` ile gönder. (~20 sn/özet)
...
8. D4: CoinGecko trending (key varsa) + web_search("fear greed whale narrative <TOKEN> <tarih>") + Gemma haber özeti (Ollama varsa)
...
> **İzleme:** `olcucu.py --symbol <SYM> --mtf` → çoklu-TF + erken belirti. Likidasyon = web_search (Apify kapalı).
...
| Gemma | D4 haber özeti | Ollama `gemma4:12b` (yerel) | 0 |
